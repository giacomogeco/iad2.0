...
wpath='/Users/giacomo/Documents/item/matlab/iad_av_detector/detections/';
...
% station=iad_read_ascii2cell(['/Users/giacomo/Documents/item/matlab/iad_av_detector/conf_files/wyssen/conf_','rp1','_2016_priv.txt']);
...
% station=iad_read_ascii2cell(['/Users/giacomo/Documents/item/matlab/iad_av_detector/conf_files/wyssen/conf_','no1','_2017_priv.txt']);
station=iad_read_ascii2cell(['/Users/giacomo/Documents/item/matlab/iad_av_detector/conf_files/wyssen/conf_','rp1','_2016_test.txt']);
...
% [detections,EVENTS,dts,evts]=iad_detections2events(DT,L,txx,prs,cc,azz,slw,rs,Fp,station.ex_shift)

load('/Users/giacomo/Documents/item/progetti/wyssen/Canada/Calibration/2016-2017/Artillery/ArtilleryG5-G8_statistics.mat')

return
%%
k=0;
clear ARTIME ARIDX DT
for i=1:length(IDAtime)   
    IT=char(IDAtime(i));
    if ~strcmp(IT,'NaN')
        idatime=datenum(IT,'dd-mmm-yyyy HH:MM:SS');
        j=find(Ev_Ex.data(:,1)-8/24>idatime+1/86400 & Ev_Ex.data(:,1)-8/24<idatime+36/86400);
        
        if ~isempty(j)
            j=j(1);
            k=k+1;
            ARTIME(k)=Ev_Ex.data(j,1)-7/24;
            GTIME(k)=idatime;
            GAZ(k)=Ev_Ex.data(j-1,8);
            ARIDX(k)=Ev_Ex.data(j,6);
            ARAZ(k)=Ev_Ex.data(j,8);
            DT(k)=86400*(ARTIME(k)-idatime);
        end
    end
end

figure,
ax(1)=subplot(211);plot(detections.time-7/24,detections.pressure,'.k')
grid on
ax(2)=subplot(212);plot(detections.time-7/24,detections.backazimuth,'.k')
hold on
plot(GTIME,GAZ,'or')
plot(ARTIME,ARAZ,'og'),grid
linkaxes(ax,'x')
return
%%
% station.nav_minpressure(1)=.1;
