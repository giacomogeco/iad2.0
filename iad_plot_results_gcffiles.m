...
% D.tt=[];D.CH1=[];D.CH2=[];D.CH3=[];D.CH4=[];

    D=iad_read_gcffile(upper(namestz),tfiles,gcfstreamID,gcfGain,station);


icut=D.tt>=tfiles(1) & D.tt<tfiles(end)+15/1440;
data = structfun(@(x) ( x(icut) ), D, 'UniformOutput', false);
tv=data.tt;
...
if size(Ev_Ex.data,1)>0,
    plot(86400*(Ev_Ex.data(:,1)-T0)+station.ex_window/2,zeros(size(Ev_Ex.data,1),1),'sy','Linewidth',2)
    hold on
    for i=1:size(Ev_Ex.data,1),
        line([86400*(Ev_Ex.data(i,1)-T0)+station.ex_window/2,86400*(Ev_Ex.data(i,2)-T0)+station.ex_window/2],...
