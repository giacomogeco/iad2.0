function [detections,EVENTS,dts,evts,prsd]=iad_detections2events_new(DT,L,txx,prs,prsd,cc,azz,slw,rs,Fp,iexp,shift)
...
% size(txx),size(prs),size(azz),size(rs),size(cc)
...
%         end
        ...
        % sesor array check
        nprs=size(ppdd,2);
        for ick=1:size(ppdd,1)
            evi=ppdd(ick,:);
            I1=[];
            for ip=1:nprs-1 
                for j=ip+1:nprs
                    I1=nansum([I1;(1-evi(ip)/evi(j))^2]);   
                end
            end
            II(ick)=2*I1./(nprs^2-nprs);
        end       
        Pd(i)=median(II);
