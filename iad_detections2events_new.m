function [detections,EVENTS,dts,evts]=iad_detections2events_new(DT,L,txx,prs,cc,azz,slw,rs,Fp,shift)
...
if isempty(txx),
    ...
    disp('no events')
    EVENTS=[];
    evts.data=vertcat(tti',ttf',D,Cm,Cd,Pmax,Pd,Am,azvar,Sm,Smax,Str,Fm,Fd,Rm,Rd,Prb,vappG,Amin,Amax);
    ...
    evts.legend={'1.Tempo inizio finestra';'2.Tempo fine finstra';'3.Durata';'4.Coerenza media';...
        ...
        '15.Consistenza media';'16.Consistenza STD';'17.Probability';'18.Velocity Gradient';...
        '19.Azimuth inizio';'20.Azimuth fine'};
    return
...
%         Sav(i)=NaN; 
        ...
        if length(twi)<9
            ...
            [vappG(i),Str(i)]=iad_detections_trends([twi;ss']',twi(1),twi(end),npol);
            
%             [Vtr(i),b] = iad_trend(ss',86400*(twi-twi(1))',2);
        end
...
%         [Str(i),azvar(i),Atrend(i)]=detections2downhillcoeff(ss,aa,L);
        ...
        if length(twi)<9
            ...
            npol=round(length(ss)/3);
            [azG(i),azvar(i)]=iad_detections_trends([twi;unwrap(aa)]',twi(1),twi(end),npol);
        end
...
events=vertcat(tti',ttf',D,Cm,Cd,Pmax,Pd,Am,azvar,Sm,Smax,Str,Fm,Fd,Rm,Rd,Prb,vappG,Amin,Amax);
