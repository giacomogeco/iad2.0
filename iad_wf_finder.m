...
function RXC=iad_wf_finder(mst,mtrx)

% size(mst)
for i=1:size(mtrx,2)-size(mst,2)
    
    rxc=zeros(1,size(mtrx,1));
    for ii=1:size(mtrx,1)
        
%         size(mtrx(ii,i:i+size(mst,2)-1))
        rx=corrcoef(mst,mtrx(ii,i:i+size(mst,2)-1));
