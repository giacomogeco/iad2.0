...
save('temp.mat','net','namestz','ConfFileName')
%%%%%%%%%% SET PATHS %%%%%%%%%%%
...
%         station.smp=[50 50 50 50];
	...
	disp('EXPLOSION')
		...
		station.ex_taper,station.ex_azmaxstd,...
		slh,working_dir,'true');
	...
	disp('AVALANCHES')
		...
		station.av_taper,station.av_azmaxstd,...
		slh,working_dir,'false');
        ...
        if Ev_Ex.data(1,1)>0
            ...
            for i=1:size(station.twr_baz,1)
                ...
                evindexes=find(Ev_Ex.data(:,8)>=station.twr_baz(i,1) & Ev_Ex.data(:,8)<=station.twr_baz(i,2) & ...
                    Ev_Ex.data(:,1)>0 & ...     %... no pressure threshold
                    Ev_Ex.data(:,18)>station.ex_minpressure(1) & ...
...
%                     Ev_Ex.data(:,1)>0 & ...     %... no pressure threshold
%                     Ev_Ex.data(:,18)>station.ex_minpressure(1) & ...
%                     Ev_Ex.data(:,18)<station.ex_minpressure(2) & ...
%                     Ev_Ex.data(:,10)<station.ex_maxappvel);
...
%     '15.Consistenza media';'16.Consistenza STD'};
        ...
        if size(Ev_Nav.data,1)>0,
            ...
            if ~isempty(iprobabilistic),
				...
				ideterministic=find(Ev_Nav.data(:,3)>station.nav_mindur(2) & ... 
					...
					Ev_Nav.data(:,19)<-0.5);	%... aggiunto il 16 Gen 2017
...
% 	iad_sendmail_avalanches_customer
	...
	if strcmp(station.name,'dvs'),
		'ok'
% 		iad_sendmail_avalanches_lgs
	else
